...
function savePolicies(policies: any[]) {
  writeJsonFile(DATA_FILE, policies);
}

function loadAuditLogs(): any[] {
  return readJsonFile(SECURITY_LOGS_FILE, [
    {
      id: "sec-1",
      timestamp: new Date().toISOString(),
      action: "SYSTEM_INITIALIZED",
      actor: "VIJAY SHIROYA (CA)",
      details: "V Shiroya AI backend initialized.",
      ipAddress: "127.0.0.1"
    }
  ]);
}
...
function normalizeResult(raw: any, pre: any) {
  ...
  const stringFields = [
    "ownerName", "policyNumber", "providerCompany", "policyType", "startDate", "endDate",
    "premiumFrequency", "insuredPerson", "nominee", "nomineeRelationship", "phoneNumber",
    "email", "address", "dateOfBirth", "agentName", "agentPhone", "branchName", "paymentMode",
    "maturityDate", "documentType", "detectedInsurer", "appliedTemplate", "ageSource"
  ];
  ...
  // Dates are normalized only when they are unambiguous. This prevents 03/04/2024
  // from being silently interpreted by the server using locale-specific rules.
  ...
  // Preserve an explicit age printed on the PDF. Only calculate it when the document
  // did not provide an age. For policy documents, calculate at commencement date first.
  if (result.age === null) {
  ...
  result.additionalDetails = Array.isArray(result.additionalDetails)
    ? result.additionalDetails
        .filter((x: any) => x && cleanValue(x.label) && cleanValue(x.value))
        .map((x: any) => ({
  ...
  result.fieldConfidenceMap = result.fieldConfidenceMap && typeof result.fieldConfidenceMap === "object"
    ? result.fieldConfidenceMap
    : {};
  result.fieldEvidence = Array.isArray(result.fieldEvidence) ? result.fieldEvidence : [];

  const requiredFields = [
    "ownerName", "policyNumber", "providerCompany", "policyType", "startDate", "endDate",
    "premiumAmount", "sumAssured", "insuredPerson", "nominee", "phoneNumber", "email", "address",
  ...
  requiredFields.forEach(field => {
    ...
    } else if (!result.fieldConfidenceMap[field]) {
      result.fieldConfidenceMap[field] = "medium";
    }
  });
  ...
  // Do not invent a policy end date. The previous implementation guessed one year when
  // the PDF did not state a term; that can produce materially wrong insurance records.
  if (result.endDate && result.maturityDate && result.endDate === result.maturityDate) {
    // Keep both fields; they may legitimately represent the same date.
  }
  ...
  if (end) {
    ...
    // Status is unknown when no expiry is printed. ACTIVE is retained for compatibility,
    // but the missing endDate is explicitly surfaced for review.
...
const EXTRACTION_SCHEMA = {
  type: Type.OBJECT,
  properties: {
    ownerName: { type: Type.STRING, description: "Exact policy owner/proposer name visible on the PDF. Do not infer." },
    policyNumber: { type: Type.STRING, description: "Exact policy number as printed." },
    providerCompany: { type: Type.STRING, description: "Exact insurance company name visible on the document." },
    policyType: { type: Type.STRING, description: "Exact plan/product/policy type visible on the document." },
    startDate: { type: Type.STRING, description: "Exact commencement/risk start date as printed. Use the visible date, not a guessed date." },
    endDate: { type: Type.STRING, description: "Exact expiry/risk end date as printed. Never calculate an end date if it is absent." },
    premiumAmount: { type: Type.NUMBER, description: "Premium amount explicitly shown on the document." },
    premiumFrequency: { type: Type.STRING, description: "Premium frequency exactly as stated." },
    sumAssured: { type: Type.NUMBER, description: "Sum assured/sum insured explicitly shown. Do not confuse with premium or IDV." },
    insuredPerson: { type: Type.STRING, description: "Name(s) of insured/life assured person(s). Preserve multiple names." },
    nominee: { type: Type.STRING, description: "Nominee name(s) exactly as shown." },
    nomineeRelationship: { type: Type.STRING, description: "Nominee relationship exactly as shown." },
    phoneNumber: { type: Type.STRING, description: "Phone/mobile number exactly as shown." },
    email: { type: Type.STRING, description: "Email exactly as shown." },
    address: { type: Type.STRING, description: "Full address exactly as shown. Preserve line information when useful." },
    dateOfBirth: { type: Type.STRING, description: "Date of birth exactly as printed. Do not confuse DOB with policy date." },
    age: { type: Type.INTEGER, description: "Age explicitly printed on the PDF, especially in insured/member tables. If not printed, return null; the server may calculate it from DOB." },
    ageSource: { type: Type.STRING, description: "Use explicitly_extracted_from_document when age is printed; otherwise null." },
    agentName: { type: Type.STRING, description: "Agent/advisor/broker name exactly as shown." },
    agentPhone: { type: Type.STRING, description: "Agent/advisor/broker phone exactly as shown." },
    branchName: { type: Type.STRING, description: "Servicing/issuing branch exactly as shown." },
    paymentMode: { type: Type.STRING, description: "Payment mode exactly as shown." },
    maturityDate: { type: Type.STRING, description: "Maturity date exactly as shown, if applicable." },
    documentType: { type: Type.STRING, description: "Document type, e.g. policy schedule, policy bond, renewal notice, premium receipt, endorsement, claim document." },
    detectedInsurer: { type: Type.STRING, description: "Insurance company identified from visible branding/text." },
    appliedTemplate: { type: Type.STRING, description: "Short layout/template description based on the actual document." },
    additionalDetails: {
      type: Type.ARRAY,
      items: {
        type: Type.OBJECT,
    ...
    extractedText: { type: Type.STRING, description: "Faithful text/transcription summary of the important visible content, including table values and fine-print identifiers." },
    fieldConfidenceMap: { type: Type.OBJECT, properties: {} },
    fieldEvidence: {
      type: Type.ARRAY,
      items: {
        type: Type.OBJECT,
        properties: {
          ...
          sourceText: { type: Type.STRING, description: "Short exact phrase or label/value pair visible on the PDF that supports the extracted field." }
        },
...
const INSURER_HINTS = [
  "LIC", "HDFC ERGO", "ICICI Prudential", "ICICI Lombard", "Star Health", "SBI Life",
  "Max Life", "Care Health", "Niva Bupa", "Bajaj Allianz", "Tata AIA", "New India Assurance",
  "Oriental Insurance", "United India Insurance", "National Insurance", "Go Digit", "Reliance General"
];
...
async function extractOnce(ai: GoogleGenAI, pdf: string, mimeType: string, fileName: string, instruction: string, pre: any) {
  const prompt = `You are an evidence-first insurance document extraction engine.
...
8. If DOB is printed but age is not, leave age null; the server will calculate age separately using policy commencement date when possible.
...
14. extractedText must be a faithful transcription-style summary of the important document content, not a generic description.
...
- Keep a candidate value only if it is visibly supported by the PDF.
- Correct OCR mistakes, wrong digit order, wrong names, wrong dates, wrong amounts and wrong relationships when the PDF clearly shows the correction.
- Never invent a value that is absent.
- Do not calculate or substitute current age when the PDF explicitly prints an age.
- If DOB is visible but age is not, leave age null.
- Check every page and every table, especially insured-member tables and nominee tables.
- Preserve multiple insured members and their ages as additionalDetails.
- Return the COMPLETE corrected JSON using the same schema.
...
async function analyzePolicyDocument(fileData: string | undefined, fileName: string, mimeType: string, instruction: string) {
  ...
  for (const modelAttempt of ["primary"]) {
    try {
      first = await extractOnce(ai, pdf, mimeType, fileName, instruction, pre);
      break;
    } catch (error: any) {
      lastError = error;
      console.warn(`Primary extraction failed (${modelAttempt}):`, error?.message || error);
    }
  }

  if (!first) throw lastError || new Error("PDF extraction failed.");

  let verified = first;
  try {
    verified = await verifyOnce(ai, pdf, mimeType, first, fileName);
  ...
  const result = normalizeResult(verified, pre);
  result.documentType = result.documentType || pre.documentType;
  result.detectedInsurer = result.detectedInsurer || pre.detectedInsurer;
  result.appliedTemplate = result.appliedTemplate || pre.appliedTemplate;

  return result;
}

app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "V Shiroya AI Backend v2", geminiConfigured: Boolean(process.env.GEMINI_API_KEY), timestamp: new Date().toISOString() });
});
...
app.post("/api/policies", (req, res) => {
  try {
    ...
    addAuditLog("POLICY_CREATED", `Saved policy #${policy.policyNumber}`, req);
    res.json({ success: true, policy });
  } catch (error: any) {
    res.status(500).json({ error: "Failed to save policy record", details: error?.message });
  }
});
...
app.put("/api/policies/:id", (req, res) => {
  ...
  savePolicies(policies);
  addAuditLog("POLICY_UPDATED", `Updated policy #${policies[index].policyNumber}`, req);
...
app.delete("/api/policies/:id", (req, res) => {
  ...
  addAuditLog("POLICY_DELETED", `Deleted policy #${existing.policyNumber}`, req);
...
app.post("/api/notifications/send-alert", (req, res) => {
  ...
  const alerts = targets.map(p => {
    ...
    const alert = {
      ...
      recipientEmail: p.email || "N/A",
      recipientPhone: p.phoneNumber || "N/A",
      channel,
      subject: `Policy renewal notice - ${p.policyNumber}`,
      body: customMessage || `Your policy ${p.policyNumber} expires in ${daysLeft} days on ${p.endDate}.`,
      status: "GENERATED",
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => console.log(`V Shiroya AI Server v2 listening on 0.0.0.0:${PORT}`));
}
